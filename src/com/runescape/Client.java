...
public class Client extends GameApplet {
	...
	private void buildInterfaceMenu(int i, Widget widget, int k, int l, int i1, int j1) {
		if (widget == null || widget.type != 0 || widget.children == null || widget.invisible)
			return;
		...
		for (int l1 = 0; l1 < size; l1++) {
			...
			if (childInterface.type == Widget.TYPE_CONTAINER) {
				...
				if (childInterface.type == Widget.TYPE_INVENTORY && !childInterface.invisible && !(childInterface.id >= 22035 && childInterface.id <= 22042)) {
					int k2 = 0;
	...
	private void drawBank(int _x, int currentY) {
		...
		if(drawPlus) {
			...
			Widget.interfaceCache[id_start + (final_loop * 4)].invisible = false;
		...
		for(int i = 0; i < scrollBar.children.length; i++) {
			...
			Widget.interfaceCache[scrollBar.children[i]].invisible = true;
		}
		...
		if(currentBankTab != 0) {
			...
			Widget.interfaceCache[50300 + currentBankTab].invisible = false;
